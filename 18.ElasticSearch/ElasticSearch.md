...
> index 和 create 的区别，index 允许覆盖，create 不允许覆盖，不允许文档 _id 重复，
操作前进行删除是必须的；否则插入会失败。

```

{ "create": { "_index": "...", "_id": "xxx" } }
{ <document body> }
...
| 需求场景                 | 推荐方式                    |
| -------------------- | ----------------------- |
| 只允许新数据插入，不允许重复 `_id` | 保留 `create` + 提前 delete |
| 允许更新同 `_id` 的旧数据     | 使用 `index` 动作，无需 delete |
| 保证幂等更新               | 使用 `index` 并带上版本字段（可选）  |
