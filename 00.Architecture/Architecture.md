...
它的作用是将我们输入的网址（如 www.example.com）转换成计算机可以识别的 IP 地址（如 93.184.216.34），从而建立网络连接。
...
浏览器首先查看系统的 hosts 文件，是否手动配置了 www.example.com 的 IP 地址。

如果有，系统直接返回该 IP，不会发起任何 DNS 查询。
...
浏览器缓存（如 Chrome、Firefox 会保存最近访问的域名）

操作系统缓存（Windows/macOS 等系统级缓存）
...
本地系统会将请求发给递归 DNS 服务器（也叫 Recursive Resolver），它通常由：

网络运营商（如电信、移动）

公共 DNS 服务（如 Google 8.8.8.8、Cloudflare 1.1.1.1）

提供。递归 DNS 也可能缓存了结果：

如果命中缓存，直接返回 IP 地址

如果仍未命中，就进入递归解析流程（以下 4 步）

从根 DNS 开始打听

递归 DNS 首先访问根 DNS 服务器（Root Server），这些服务器是互联网的 DNS 查询入口，由 ICANN 协调管理，全球共有 13 组。

根服务器并不会直接告诉你 IP，但会指出 .com 顶级域名的服务器地址。

请求顶级域名服务器（TLD）

接着，递归 DNS 访问 .com 的顶级域名服务器（TLD Server），它负责管理 .com 后缀的域名。

它会告诉你 example.com 由哪个权威 DNS 管理。

查询权威 DNS 服务器

递归 DNS 最后访问 example.com 的权威 DNS 服务器（Authoritative Server），这台服务器真正掌握 www.example.com 的解析记录。

它会返回域名的 IP 地址（A 记录 → IPv4，AAAA 记录 → IPv6）。

如果遇到 CNAME（别名记录），会先返回真实域名，再继续查询。
...
将 IP 返回给本地系统 → 操作系统缓存 → 浏览器使用 → 正式发起连接

通俗类比助记：
可以将 DNS 查询过程比作“打电话找人”的过程：

浏览器像是你，想找“张三”（域名），但你不知道他电话号码（IP）。

你先查手机通讯录（hosts 文件、本地缓存），找不到？

那你找中介（递归 DNS）帮忙打听，中介去问：

根 DNS（国家档案馆） → 哪个部门管 .com？

TLD（.com 管理处） → 谁负责 example.com？

权威 DNS（房东） → 给出 www.example.com 的真实地址！
