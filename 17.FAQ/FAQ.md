...
大多数情况下，打开这种文件，`vim`的状态栏会显示文件格式：`utf-8[dos]`或者`utf-8[mac]`，此时只需要下面命令可转为`unix`格式，即可删除或转换所有的`^M`:

```
:set ff=unix
```
如果`vim`显示文件格式已经是`utf-8[unix]`，这时候上述命令就不管用了，说明 `vim` 识别类型错误，可以先把它纠正（即用`dos`或`mac`格式打开当前文件），再变更类型：
