...
35、数据库的索引结构说一下，为什么MySQL用B+树做索引而不用B树?
...
- 数据只出现在叶子节点，非叶子节点不存储data，只存储索引（冗余）。非叶子节点存储的索引更多，如果存储相同级别的数据量，B+树的高度小。
...
如果设置了主键，那么`InnoDB`会选择主键作为聚簇索引。如果没有显式定义主键，则`InnoDB`会选择第一个不包含有`NULL`值的唯一索引作为主键索引；如果也没有这样的唯一索引，则`InnoDB`会选择内置6字节长的`ROWID`作为隐含的聚集索引(`ROWID`随着行记录的写入而主键递增)。如果不使用主键无形会增加数据库的计算成本。
