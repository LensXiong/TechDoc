...
![websocket_http.png](websocket_http.png)
...
静态地址分配是一种在网络中手动配置和分配固定的 IP 地址给特定设备的方法。与动态地址分配（使用 DHCP ）不同，静态地址分配要求管理员手动为每个设备指定一个特定的IP地址，而不是由 DHCP 服务器自动分配。
